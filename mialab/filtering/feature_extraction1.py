...
class Hog_descriptor(fltr.IFilter):
    ...
    def execute(self, image: sitk.Image, params: fltr.IFilterParams = None) -> sitk.Image:
    # slicewise extraction

        img_arr = sitk.GetArrayFromImage(image)

        depth, height, width = img_arr.shape

        hog_slicestack = np.empty_like(img_arr)

        # slice-wise extraction
        for idx in range(0, depth):
            slice_arr = img_arr[idx, :, :]

            fd, hog_slicestack[idx, :, :] = hog(slice_arr, orientations=8, pixels_per_cell=(16, 16), cells_per_block=(1, 1),
